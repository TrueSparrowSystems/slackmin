...
const rootPrefix = '../..',
  ...
  slackHelper = require(rootPrefix + '/lib/slack/helper'),
...
class Message {
   ...
   *
   * @param {array} buttonDetails (array of objects with keys - buttonText, value)
   *
   * @sets oThis.messageJSON
   */
   ...
   */
  addCustomHeader(adminName, actionText) {
    const oThis = this;

    oThis.addDivider();

    oThis.addSection(`*ADMIN:* ${adminName} - *ACTION:* ${actionText}`);
  }
   ...
   */
  async sendMessageToChannel(postMessageParams) {
    const oThis = this;
