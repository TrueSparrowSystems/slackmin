...
class Modal {
   ...
   * @param {boolean} [isOptional]
   */
  addTextboxWithPlaceholder(labelText, placeHolderText, multiline = true, isOptional = false) {
    const oThis = this;
   ...
   * @param {array<object>} [initialOptionsArray]
   */
  addCheckBoxes(labelText, optionsArray, initialOptionsArray = []) {
    const oThis = this;
    ...
    for (let index = 0; index < initialOptionsArray.length; index++) {
      const initialOption = initialOptionsArray[index];
